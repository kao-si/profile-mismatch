...
dat_post %>%
filter(is.na(rname), !is.na(rmale)) %>%
select(post_id, rname, rmale)

# No gender identification for some names
dat_post %>%
filter(!is.na(rname), is.na(rmale)) %>%
select(post_id, rname, rmale)
...
dat_post %>%
filter((str_detect(rname, "先生") & (!rmale == "yes" | is.na(rmale))) |
(str_detect(rname, "女士") & (!rmale == "no" | is.na(rmale)))) %>%
select(post_id, rname, rmale)
```